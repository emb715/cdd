...
User may request:
- `/cdd:list-work` - List all work items
- `/cdd:list-work --status=in-progress` - Filter by status
- `/cdd:list-work --type=bug` - Filter by type
- `/cdd:list-work --priority=high` - Filter by priority
- `/cdd:list-work --status=draft --type=feature` - Multiple filters
...
├──────┼────────────────────────────┼──────────┼────────────┼──────────┼────────┼────────┼──────────┤
│ 0001 │ User Authentication System │ Feature  │ Complete   │ High     │ 5      │ 8.5    │ 42/42   │
│ 0002 │ Dark Mode Toggle           │ Feature  │ In Progress│ Medium   │ 2      │ 3.5    │ 8/15    │
│ 0003 │ API Rate Limiting          │ Feature  │ Draft      │ High     │ --     │ --     │ --      │
├──────┼────────────────────────────┼──────────┼────────────┼──────────┼────────┼────────┼──────────┤
│ B001 │ Login Timeout Issue        │ Bug      │ In Progress│ Critical │ 1      │ 1.2    │ 3/6     │
│ B002 │ Profile Image Upload       │ Bug      │ Draft      │ Medium   │ --     │ --     │ --      │
├──────┼────────────────────────────┼──────────┼────────────┼──────────┼────────┼────────┼──────────┤
│ R001 │ Database Query Optimization│ Refactor │ Draft      │ Low      │ --     │ --     │ --      │
├──────┼────────────────────────────┼──────────┼────────────┼──────────┼────────┼────────┼──────────┤
│ S001 │ GraphQL vs REST Evaluation │ Spike    │ Complete   │ Medium   │ 3      │ 4.0    │ 12/12   │
├──────┼────────────────────────────┼──────────┼────────────┼──────────┼────────┼────────┼──────────┤
│ E001 │ V2 Platform Redesign       │ Epic     │ In Progress│ Critical │ 4      │ 6.1    │ 18/30   │
└──────┴────────────────────────────┴──────────┴────────────┴──────────┴────────┴────────┴──────────┘

Use `/show-work [ID]` to see details
```

`--` indicates data not yet captured (e.g., no sessions logged).
...
🆔 0001 | User Authentication System
   ...
   ⏱️  Duration: 5 sessions, 8.5 hours
   🔁 Context Reacquisition: 6.0 min avg
   📈 Planned Phases: 6
...
🆔 0002 | Dark Mode Toggle
   ...
   🔁 Context Reacquisition: 8.5 min avg
   📈 Planned Phases: 6
...
Metrics Snapshot:
  ...
  ⏳ Total Hours Logged: 23.3
  🔁 Avg Context Reacquisition: 6.2 min
  ✅ Tasks Completed: 83 / 105 (79%)
  📋 Planned Parent Phases: 6
...
> Tip: Run `node scripts/collect-metrics.js` periodically to regenerate JSON rollups for dashboards or reports.
```
...
High Priority Items:
  → Work 0003 (API Rate Limiting) is Draft - Run /cdd:plan-work 0003
...
Recently Updated:
  → Work 0002 updated today - Continue work with /cdd:save-session 0002

Ready to Complete:
  → Work 0006 at 95% - Consider /cdd:complete-work 0006

💡 Quick Commands:
  /show-work [ID]     - View work item details
  /cdd:plan-work [ID]     - Generate implementation plan
  /cdd:save-session [ID]  - Save session progress
  /cdd:complete-work [ID] - Mark as complete
```
...
**User:** `/cdd:list-work --status=in-progress`
...
**User:** `/cdd:list-work --type=bug --priority=critical`
...
Next step: /cdd:save-session B001 to track fix progress
```
...
Examples:
  ...
  /cdd:create-work fix login timeout bug
  /cdd:create-work refactor database queries
...
/show-work [ID]        → View details
   ↓
/cdd:plan-work [ID]        → Generate plan
   ↓
/cdd:save-session [ID]     → Track work
   ↓
/cdd:complete-work [ID]    → Finish work
```
...
Morning routine:
  ...
  → Use /cdd:save-session when done

Planning routine:
  /cdd:list-work --status=draft
  → See what needs planning
  → Run /cdd:plan-work on priority items
